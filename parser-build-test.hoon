::  hoon.hoon with all subsections of Layer 5 removed
::  except for 5d, the parser. In my mind, this should build:
::
::  =text .^(@t %cx %/parser/hoon)
::  (slap !>(~) (ream text))
::
::  if not, what dependencies does the parser have on the compiler?
::
::  unfortunately, this actually just dig: memes in the dojo
::  Layer 4 has a ton of parser definitions in it too; attempting
::  to dissect the compiler into its component parts, rather than
::  grapple with it in place, might not be the best approach
::
::::    /sys/hoon                                       ::
  ::                                                    ::
=<  ride
